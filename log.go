...
/*
Four types of logs
1: Debug log, output to ../log/debug.Date.log, keep at most three days
2: Warning log, output to ../log/warn.log
3: Status log, output to ../log/stat.log
4: User log, output to ../log/u_XXX.log, where XXX is the uid
*/
...
type log_t struct {
	perfs    *sync.Map
	mdy_flag int //0: unchanged, 1: last time unchanged, 2: changed
	statf    *os.File
	warnf    *os.File
	logdir   string
	svrname  string
...
// Default by day, if you want by hour/month, you need to set this function in advance
var LogPrefixFn = LogPrefixDay

// Default keep three log files, if you want more, you need to set this value in advance
var LogKeepFiles = 3
...
func LogWrite(w io.Writer, level string, format string, v ...interface{}) {
	var nowTime string
	if level == "1" { //only debug log without time
...
func StatKeepMin(key string, val int64) {
	...
	if loaded {
		...
		for { //sacrifice performance for accuracy
...
func StatKeepMax(key string, val int64) {
	...
	if loaded {
		...
		for {
			old := atomic.LoadInt64(&pp.total_val)
			if old >= val || atomic.CompareAndSwapInt64(&pp.total_val, old, val) {
				break //if already maximum, or swap successful, then break
			}
		}
	}
}
